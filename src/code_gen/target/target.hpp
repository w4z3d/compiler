...
enum class CompilerTarget { X86_64 };

class Target {
public:
  enum class Endianness { BIG, SMALL };
  [[nodiscard]] virtual const std::vector<mir::PhysicalRegister>
  get_gprs() const = 0;
  // in bits
  [[nodiscard]] virtual size_t get_word_size() const = 0;
  // in bits
  [[nodiscard]] virtual size_t get_pointer_size() const = 0;
  [[nodiscard]] virtual mir::PhysicalRegister
  get_instruction_pointer() const = 0;
